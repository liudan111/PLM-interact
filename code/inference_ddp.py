...
class CrossEncoder():
    ...
    def predict(self, args,
        ...
        test_dataloader = DataLoader(test_samples, batch_size=batch_size_val,sampler= test_sampler,collate_fn = 
    self.smart_batching_collate,shuffle=False)
        ...
        if self.master_process:
            ...
            csv_headers = ['epoch', 'hostname',"test_loss", "precision-recall auc", 'f1', 'precision', 'recall', 'f1_threshold',  'max_acc', 'best_threshold']
            ...
            with open(csv_path, mode="a" if output_file_exists else 'w', encoding="utf-8") as f:
                ...
                if not output_file_exists:
                    writer.writerow(csv_headers)
                writer.writerow([self.best_epoch, hostname, loss.item(), ap_score,f1, precision, recall, f1_threshold, max_acc, best_threshold])  
